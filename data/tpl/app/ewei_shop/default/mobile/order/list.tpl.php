...
<style type="text/css">
    ...
    .order_topbar { display: flex; display: -webkit-flex; height:44px; width:100%; background:#fff; border-bottom:1px solid #e3e3e3;}
    .order_topbar .nav {height:44px; width: 100%; /*<?php  if($_GPC['status']!=4) { ?>width:16%;<?php  } else { ?>width:50%;<?php  } ?>*/ line-height:44px; text-align:center; font-size:14px; float:left; color:#666;}
...
<script id='tpl_order_list' type='text/html'>
    ...
    <div class="order_topbar">
		...
		<div class="nav <?php  if($_GPC['status']=='6') { ?>on<?php  } ?>"  data-status="6">待制作</div>  
		<div class="nav <?php  if($_GPC['status']=='7') { ?>on<?php  } ?>"  data-status="7">制作中</div> 		
        <div class="nav <?php  if($_GPC['status']=='2') { ?>on<?php  } ?>"  data-status="2">运输中</div>
        ...
        <div class="sub">
          ...
          <%/if%>
            ...
            <%if order.canrefund%>
                    ...
                    <%else%>
                       <div  state="<%order.allstate%>" status="<%order.status%>"  class="sub1 order_refund">申请退款</div>
                  <%/if%>
            <%/if%>
