...
if ($op == 'display') {
    ...
    if ($_W['ispost']) {
        if ($apido == 'savepage') {
            ...
            unset($data);
            $insert = array('pagename' => $pagename, 'pagetype' => $pagetype, 'pageinfo' => base64_encode(json_encode($p)), 'savetime' => $date, 'datas' => base64_encode(json_encode($datas)), 'uniacid' => $_W['uniacid'], 'keyword' => $keyword);
            if (empty($id)) {
                ca('designer.page.edit');
