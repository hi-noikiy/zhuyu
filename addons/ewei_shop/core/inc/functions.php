...
{
	$_var_40 = get_auth();
	return empty($_var_40["code"]) ? '' : $_var_40["code"];
}
function get_auth()
{
	global $_W;
	$_var_41 = pdo_fetch("select sets from " . tablename("ewei_shop_sysset") . " order by id asc limit 1");
	$_var_42 = iunserializer($_var_41["sets"]);
	if (is_array($_var_42)) {
		return is_array($_var_42["auth"]) ? $_var_42["auth"] : array();
	}
	return array();
}
function check_shop_auth($_var_14 = '', $_var_29 = 's')
{
	global $_W, $_GPC;
	return ;
}
