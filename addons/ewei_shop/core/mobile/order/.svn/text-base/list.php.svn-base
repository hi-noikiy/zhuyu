	...
	if ($operation == 'display') {
		...
		if ($status != '') {
			if ($status != 4) {
				...
				if($status == 6){
					$condition .= 'and allstate=0 and status=1';
				}
				else if($status == 7){
					$condition .= 'and allstate=1 and status=' . intval(1);
				}
				else if ($status == 2) {
		...
		$list = pdo_fetchall('select id,addressid,ordersn,price,status,iscomment,isverify,verified,verifycode,iscomment,refundid,expresscom,express,expresssn,finishtime,virtual,paytype,allstate,expresssn,refundstate from ' . tablename('ewei_shop_order') . " where 1 {$condition} order by createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
