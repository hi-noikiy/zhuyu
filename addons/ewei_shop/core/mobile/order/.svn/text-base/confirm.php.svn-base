    ...
    if ($operation == 'display') {
        ...
        if ($operation == 'getdispatchprice') {
            ...
            if ($operation == 'create' && $_W['ispost']) {
                ...
                $order = array('uniacid' => $uniacid, 'openid' => $openid, 'ordersn' => $ordersn, 'price' => $totalprice, 'cash' => $cash, 'discountprice' => $discountprice, 'deductprice' => $deductmoney, 'deductcredit' => $deductcredit, 'deductcredit2' => $deductcredit2, 'deductenough' => $deductenough, 'status' => 0, 'paytype' => 0, 'transid' => '', 'remark' => $_GPC['remark'],'birthdaycard' => $_GPC['birthdaycard'],'cakehat' => $_GPC['cakehat'],'birthdaycandle' => $_GPC['birthdaycandle'],'fruitid' => $_GPC['fruitid'],'seltime' => $_GPC['seltime'], 'addressid' => empty($dispatchtype) ? $addressid : 0, 'goodsprice' => $goodsprice, 'dispatchprice' => $dispatch_price, 'dispatchtype' => $dispatchtype, 'dispatchid' => $dispatchid, 'storeid' => $carrierid, 'carrier' => $carriers, 'createtime' => time(), 'isverify' => $isverify ? 1 : 0, 'verifycode' => $verifycode, 'virtual' => $virtualid, 'isvirtual' => $isvirtual ? 1 : 0, 'oldprice' => $totalprice, 'olddispatchprice' => $dispatch_price, 'couponid' => $couponid, 'couponprice' => $couponprice);
                if ($diyform_plugin) {
                ...
                if ($deductcredit2 > 0) {
                    $shop = m('common')->getSysset('shop');
                    m('member')->setCredit($openid, 'credit2', -$deductcredit2, array('0', $shop['name'] . "购物余额抵扣: {$deductcredit2} 订单号: {$ordersn}"));
                }
                if (empty($virtualid)) {
