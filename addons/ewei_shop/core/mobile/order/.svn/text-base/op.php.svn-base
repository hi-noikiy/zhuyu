...
if ($_W['isajax']) {
    if ($operation == 'cancel') {
        ...
        $order = pdo_fetch('select id,ordersn,openid,status,deductcredit,deductcredit2,deductprice,couponid,remark from ' . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
        if (empty($order)) {
            show_json(0, '订单未找到!');
        }
        if ($order['status'] > 0) {
...
' . $reason;
        ...
        pdo_update('ewei_shop_order', $data, array('id' => $order['id'], 'uniacid' => $uniacid));
