...
if ($operation == 'display') {
            ...
            if ($status == '4') {
                ...
                if ($status == '5') {
                    ...
                    if ($status == '6'  ||   $status == '7' ) {
                    	if($status=='6'){
                    		$statuscondition = ' AND ( o.status = 1  and  o.allstate=0    or (o.status=0 and o.paytype=3) )';
                    	}
						else if($status=='7'){
							$statuscondition = ' AND ( o.status = 1  and  o.allstate=1    or (o.status=0 and o.paytype=3) )';
						}
    ...
    $orderstatus = array('-1' => array('css' => 'default', 'name' => '已关闭'), '0' => array('css' => 'danger', 'name' => '待付款'), '1' => array('css' => 'info', 'name' => '待发货'),'6' => array('css' => 'info', 'name' => '待制作'),'7' => array('css' => 'info', 'name' => '制作中'), '2' => array('css' => 'warning', 'name' => '运输中'), '3' => array('css' => 'success', 'name' => '已完成'));
...
} elseif ($operation == 'setmake') {
    
    $orderid = intval($_GPC['id']);
    pdo_update('ewei_shop_order', array('allstate' => 1), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
    plog('order.op.delete', "订单设置成功 ID: {$id}");
    message('设置成功', $this->createWebUrl('order', array('op' => 'display')), 'success');
...
function getList1($_var_2, $_var_3)
{
...
function order_list_close($_var_21)
{
    ...
    if ($_var_21['deductcredit2'] > 0) {
        $_var_27 = m('common')->getSysset('shop');
        m('member')->setCredit($_var_21['openid'], 'credit2', $_var_21['deductcredit2'], array('0', $_var_27['name'] . "购物返还抵扣余额 余额: {$_var_21['deductcredit2']} 订单号: {$_var_21['ordersn']}"));
    }
    if (p('coupon') && !empty($_var_21['couponid'])) {
...
' . $_GPC['reson']), array('id' => $_var_21['id'], 'uniacid' => $_W['uniacid']));
                ...
                if ($_var_28 == 10) {
                    ...
                    if ($_var_28 == 1) {
                        ...
                        foreach ($_var_38 as $_var_40) {
                            $_var_41 = trim($_var_40['credit']);
                            if (!empty($_var_41)) {
                                if (strexists($_var_41, '%')) {
                                    $_var_39 += intval(floatval(str_replace('%', '', $_var_41)) / 100 * $_var_40['realprice']);
                                } else {
                        ...
                        if ($_var_21['paytype'] == 1) {
                            ...
                            if ($_var_21['paytype'] == 21) {
                                ...
                                $_var_43 = m('finance')->refund($_var_21['openid'], $_var_8, $_var_22['refundno'], $_var_21['price'] * 100, $_var_37 * 100);
                                $_var_42 = 2;
                            } else {
                                if ($_var_37 < 1) {
                                ...
                                $_var_37 = round($_var_37 - $_var_21['deductcredit2'], 2);
                                $_var_43 = m('finance')->pay($_var_21['openid'], 1, $_var_37 * 100, $_var_22['refundno'], $_var_27['name'] . "退款: {$_var_37}元 订单号: " . $_var_21['ordersn']);
                                $_var_42 = 1;
                            }
                        }
                        if (is_error($_var_43)) {
                            message($_var_43['message'], '', 'error');
                        }
                        if ($_var_39 > 0) {
                            m('member')->setCredit($_var_21['openid'], 'credit1', -$_var_39, array(0, $_var_27['name'] . "退款扣除购物赠送积分: {$_var_39} 订单号: " . $_var_21['ordersn']));
                        }
                        if ($_var_21['deductcredit'] > 0) {
                            m('member')->setCredit($_var_21['openid'], 'credit1', $_var_21['deductcredit'], array('0', $_var_27['name'] . "购物返还抵扣积分 积分: {$_var_21['deductcredit']} 抵扣金额: {$_var_21['deductprice']} 订单号: {$_var_21['ordersn']}"));
                        }
                        ...
                        foreach ($_var_38 as $_var_40) {
                            $_var_44 = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $_var_40['id'], ':uniacid' => $_var_30));
                            pdo_update('ewei_shop_goods', array('salesreal' => $_var_44), array('id' => $_var_40['id']));
                        }
