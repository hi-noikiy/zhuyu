...
if ($operation == 'save') {
    ...
    if (!empty($tempname)) {
        die(json_encode(array('templates' => array_keys($set['shop']['ordertemplates']), 'tempname' => $tempname)));
    }
    ...
    if ($operation == 'delete') {
        ...
        $setdata = pdo_fetch('select * from ' . tablename('ewei_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
        m('cache')->set('sysset', $setdata);
        die(json_encode(array('templates' => array_keys($set['shop']['ordertemplates']))));
    } else {
        if ($operation == 'gettemplate') {
            ...
            die(json_encode(array('columns' => $columns, 'others' => $others)));
        } else {
            if ($operation == 'reset') {
